...
import { shouldRetryCodexPoolAccountQuota } from "./core-codex-account";
...
async function resolveAlternateCompactContext(args: {
  ...
  try {
    ...
    const authCtx = await resolveAvailableCodexAuthContext(req.headers, config, route.codexAccountMode, {
      ...
      excludeAccountId: excludeAccountId ?? MAIN_CODEX_ACCOUNT_ID,
...
export async function handleResponsesCompact(
  ...
  if (supportsNativeResponsesCompactEndpoint(route.providerName, route.provider) && !accountGatedCompactWireModel && !route.combo
    ...
    const sendSingleCompactAttempt = (
      ...
      // A quota or credential refusal is about one account; the next account's quota is
      // independent. That move is charged when the budget has room, and the request's attempt
      // list (each identity once, 120 s) bounds it when it does not.
      if (!decision.allowed) {
        return accountScoped ? doFetch() : Promise.reject(new SendBudgetExhaustedError(safeHostLabel(compactUrl)));
      }
      if (!decision.permit.use() && !accountScoped) return Promise.reject(new SendBudgetExhaustedError(safeHostLabel(compactUrl)));
      return doFetch();
    };
    const sendCompactAttempt = (
      ...
      return recovery === "single"
        ? sendSingleCompactAttempt(() => codexAttempts.run(req.signal, signal => doFetch(undefined, signal)), accountScoped)
    ...
    if (
      ...
      if (!replay.ok) {
        ...
        // The refresh failed for THIS account only; the next eligible identity may serve.
    ...
    const accountQuotaFailure = !isNonReplayableResponse(upstream)
      && await shouldRetryCodexPoolAccountQuota(upstream, req.signal);
    if (
      ...
      if (alternate) {
        ...
        try {
          upstream = await sendCompactAttempt(alternate.provider, alternate.headers, "single", alternate.authCtx, true);
        } catch (err) {
          if (req.signal.aborted) {
            recordCompactPoolOutcome(outcomeCtx, 499);
            return formatErrorResponse(499, "client_cancelled", "Client cancelled compact request");
        ...
        // The alternate's own answer gets the same recovery as the first one did.
